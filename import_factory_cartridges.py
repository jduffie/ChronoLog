#!/usr/bin/env python3
"""
Script to import factory cartridge data from CSV files to the factory_cartridge_specs table.
"""
...
def import_factory_cartridges_from_csv():
    """Import factory cartridge data from CSV files in cartridges/factory/datasets"""
    ...
    csv_dir = Path(__file__).parent / "cartridges" / "factory" / "datasets"
    ...
    # Get all CSV files
    csv_files = list(csv_dir.glob("*_cartridges.csv"))
    
    if not csv_files:
        print(f"Error: No cartridge CSV files found in {csv_dir}")
        return False
    ...
    for csv_file in sorted(csv_files):
        ...
        try:
            ...
            for i in range(0, len(processed_records), batch_size):
                ...
                try:
                    # Insert batch
                    response = supabase.table("factory_cartridge_specs").insert(batch).execute()
                    ...
                    for record in batch:
                        try:
                            response = supabase.table("factory_cartridge_specs").insert([record]).execute()
                            if response.data:
                                file_inserted += 1
                            else:
                                file_skipped += 1
...
if __name__ == "__main__":
    success = import_factory_cartridges_from_csv()
