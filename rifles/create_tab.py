...
def render_create_rifle_tab(user, supabase):
    ...
    with st.form("create_rifle_form"):
        ...
        if submitted:
            ...
            # Clean up inputs
            name = name.strip()
            barrel_twist_ratio = barrel_twist_ratio.strip() if barrel_twist_ratio else None
            ...
            try:
                ...
                if "duplicate key value violates unique constraint" in str(e):
                    st.error("❌ A rifle with this name already exists. Please choose a different name.")
                else:
    ...
    for example in examples:
        ...
        st.markdown(f"- Twist: {example['barrel_twist_ratio']}, Length: {example['barrel_length']}")
        st.markdown(f"- Offset: {example['sight_offset']}, Trigger: {example['trigger']}")
