...
def render_view_rifle_tab(user, supabase):
    ...
    try:
        ...
        response = supabase.table("rifles").select("*").eq("user_email", user["email"]).order("created_at", desc=True).execute()
        
        if not response.data:
            st.info("📭 No rifle entries found. Go to the 'Create' tab to add your first rifle.")
            return
        ...
        with col2:
            ...
            selected_completeness = st.selectbox("Filter by Completeness:", completeness_options)
        
        # Apply filters
        filtered_df = df.copy()
        
        if selected_twist != "All":
            filtered_df = filtered_df[filtered_df['barrel_twist_ratio'] == selected_twist]
        ...
        display_df = display_df[[
            'name', 'barrel_twist_ratio', 'barrel_length', 'sight_offset', 
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Twist Ratio": st.column_config.TextColumn("Twist Ratio", width="small"),
                "Barrel Length": st.column_config.TextColumn("Barrel Length", width="small"),
                "Sight Offset": st.column_config.TextColumn("Sight Offset", width="small"),
        ...
        with st.expander("View individual rifle details"):
            ...
            if selected_rifle_name:
                rifle_data = filtered_df[filtered_df['name'] == selected_rifle_name].iloc[0]
                ...
                with col1:
                    ...
                    st.write(f"**Twist Ratio:** {rifle_data['barrel_twist_ratio'] or 'Not specified'}")
                    st.write(f"**Barrel Length:** {rifle_data['barrel_length'] or 'Not specified'}")
                    st.write(f"**Sight Offset:** {rifle_data['sight_offset'] or 'Not specified'}")
                
                with col2:
                    st.markdown("**Components**")
                    st.write(f"**Trigger:** {rifle_data['trigger'] or 'Not specified'}")
                    st.write(f"**Scope:** {rifle_data['scope'] or 'Not specified'}")
                    st.write(f"**Created:** {pd.to_datetime(rifle_data['created_at']).strftime('%Y-%m-%d %H:%M')}")
                    st.write(f"**Updated:** {pd.to_datetime(rifle_data['updated_at']).strftime('%Y-%m-%d %H:%M')}")
                ...
                if selected_rifle:
                    ...
                    with col1:
                        if st.button("🗑️ Delete", type="secondary"):
                            try:
                                # Delete the rifle
                                delete_response = supabase.table("rifles").delete().eq("id", selected_rifle[1]).execute()
