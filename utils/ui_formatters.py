...
# Table-specific formatters that return numeric values only (no units)
# Units are included in column headers instead

def format_speed_for_table(speed_mps: Optional[float], user_unit_system: str = "Imperial") -> str:
    """Format speed for table display (no units - units in column header)"""
    if speed_mps is None:
        return ""

    if user_unit_system == "Imperial":
        speed_fps = mps_to_fps(speed_mps)
        return f"{speed_fps:.1f}"
    else:
        return f"{speed_mps:.2f}"


def format_energy_for_table(energy_j: Optional[float], user_unit_system: str = "Imperial") -> str:
    """Format kinetic energy for table display (no units - units in column header)"""
    if energy_j is None:
        return ""

    if user_unit_system == "Imperial":
        energy_ftlb = joules_to_ftlb(energy_j)
        return f"{energy_ftlb:.1f}"
    else:
        return f"{energy_j:.1f}"


def format_power_factor_for_table(pf_kgms: Optional[float], user_unit_system: str = "Imperial") -> str:
    """Format power factor for table display (no units - units in column header)"""
    if pf_kgms is None:
        return ""

    if user_unit_system == "Imperial":
        pf_grainft = kgms_to_grainft(pf_kgms)
        return f"{pf_grainft:.0f}"
    else:
        return f"{pf_kgms:.2f}"


def format_temperature_for_table(temp_c: Optional[float], user_unit_system: str = "Imperial") -> str:
    """Format temperature for table display (no units - units in column header)"""
    if temp_c is None:
        return ""
...
def format_pressure_for_table(pressure_hpa: Optional[float], user_unit_system: str = "Imperial") -> str:
    """Format pressure for table display (no units - units in column header)"""
    if pressure_hpa is None:
        return ""

    if user_unit_system == "Imperial":
        pressure_inhg = hpa_to_inhg(pressure_hpa)
        return f"{pressure_inhg:.2f}"
    else:
        return f"{pressure_hpa:.1f}"