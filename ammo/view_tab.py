...
def render_view_ammo_tab(user, supabase):
    ...
    try:
        ...
        response = supabase.table("ammo").select("*").eq("user_email", user["email"]).order("created_at", desc=True).execute()
        
        if not response.data:
            st.info("📭 No ammo entries found. Go to the 'Create' tab to add your first ammo entry.")
            return
        ...
        with st.expander("Delete an ammo entry"):
            ...
            for _, row in filtered_df.iterrows():
                entry_label = f"{row['make']} {row['model']} - {row['caliber']} - {row['weight']}"
                ...
                if selected_entry:
                    ...
                    with col1:
                        if st.button("🗑️ Delete", type="secondary"):
                            try:
                                # Delete the entry
                                delete_response = supabase.table("ammo").delete().eq("id", selected_entry[1]).execute()
