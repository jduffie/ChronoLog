...
def render_create_ammo_tab(user, supabase):
        ...
        if submitted:
            ...
            try:
                ...
                if "duplicate key value violates unique constraint" in str(e):
                    st.error("❌ This ammo entry already exists. Please check your entries.")
                else:
    ...
    examples = [
        {"make": "Hornady", "model": "ELD-M", "caliber": "6.5 Creedmoor", "weight": "147gr"},
        {"make": "Federal", "model": "Gold Medal", "caliber": ".308 Winchester", "weight": "168gr"},
        {"make": "Winchester", "model": "Ballistic Tip", "caliber": ".223 Remington", "weight": "55gr"}
    ]
    
    for example in examples:
        st.markdown(f"- **{example['make']} {example['model']}** - {example['caliber']} - {example['weight']}")