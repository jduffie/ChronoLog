...
class SubmissionController:
    ...
    def _handle_delete_confirmation(
        self, user: Dict[str, Any], ranges: List[Dict[str, Any]]
    ) -> None:
            ...
            try:
                ...
                if success:
                    self.view.display_success_message(
                        f"✅ Successfully deleted {len(range_ids)} range(s)!"
                    )
                    ...
                    self.view.display_error_message(
                        "❌ Failed to delete some ranges. Please try again."
                    )
    ...
    def _run_submission_functionality(self, user, supabase):
        ...
        try:
            ...
            if table_result["action"] == "delete" and table_result["selected_indices"]:
                ...
                st.session_state["delete_selected_ranges"] = table_result[
                    "selected_indices"
                ]
                st.rerun()
                ...
                self.view.display_map_section(
                    user_ranges, table_result["selected_indices"]
                )
            else:
            ...
            self.view.display_error_message(
                f"Error loading your submitted ranges: {str(e)}"
            )
