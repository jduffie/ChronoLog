...
def display_profile_settings(user_profile):
    ...
    with col2:
        ...
        if st.session_state.get("edit_profile", False):
            st.info("📝 You can edit your profile below. The form will appear in the main content area.")
...
def display_account_info(user_profile):
    ...
    with col2:
        ...
        try:
            ...
            result = supabase.table("ranges_submissions").select("id", count="exact").eq("user_email", user_profile["email"]).execute()
    ...
    with col3:
        profile_complete = "✅ Complete" if user_profile.get('profile_complete') else "❌ Incomplete"
    ...
    account_info = {
        ...
        "Account Created": user_profile.get('created_at', 'Unknown')[:19] if user_profile.get('created_at') else 'Unknown',
        "Last Updated": user_profile.get('updated_at', 'Unknown')[:19] if user_profile.get('updated_at') else 'Unknown',
...
def update_unit_preference(user_profile, new_unit_system):
    ...
    try:
        ...
        success = controller.model.update_user_profile(user_profile["email"], update_data)
