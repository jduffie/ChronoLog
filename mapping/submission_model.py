...
class SubmissionModel:
    ...
    def get_user_submission_by_id(self, user_email: str, submission_id: str, supabase_client) -> Dict[str, Any]:
        """Get a specific submission by ID for a user."""
        try:
            result = supabase_client.table("ranges_submissions").select("*").eq("id", submission_id).eq("user_email", user_email).execute()
            return result.data[0] if result.data else {}
        except Exception as e:
            print(f"Error getting user submission: {e}")
            return {}

    def update_user_submission(self, user_email: str, submission_id: str, updates: Dict[str, Any], supabase_client) -> bool:
        """Update a user's submission."""
        try:
            result = supabase_client.table("ranges_submissions").update(updates).eq("id", submission_id).eq("user_email", user_email).execute()
