...
class AdminModel:
    ...
    def get_submissions_by_status(self, status: str, supabase_client) -> List[Dict[str, Any]]:
        ...
        try:
            result = supabase_client.table("ranges_submissions").select(
                ...
                "start_altitude_m, end_altitude_m, distance_m, azimuth_deg, elevation_angle_deg, "
                "display_name, submitted_at, status, review_reason, address_geojson"
            ).eq("status", status).order("submitted_at", desc=False).execute()
            return result.data if result.data else []
        except Exception as e:
    ...
    def get_submission_by_id(self, submission_id: str, supabase_client) -> Dict[str, Any]:
        """Get a specific submission by ID."""
        try:
            result = supabase_client.table("ranges_submissions").select("*").eq("id", submission_id).execute()
            return result.data[0] if result.data else {}
        except Exception as e:
            print(f"Error getting submission by ID: {e}")
            return {}

    def update_submission_review(self, submission_id: str, status: str, review_reason: str, supabase_client) -> bool:
