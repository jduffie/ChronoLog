...
class PublicRangesView:
        ...
        try:
            ...
            if isinstance(
                ...
                if features and len(features) > 0:
                    ...
                    if (
                        "properties" in firing_position
                        and "address" in firing_position["properties"]
                    ):
    ...
    def display_public_ranges_table_readonly(
    ...
    def display_public_ranges_table_admin(
        self, ranges: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _display_public_ranges_table(
        self, ranges: List[Dict[str, Any]], is_admin: bool = False
    ) -> Dict[str, Any]:
        ...
        with col1:
            ...
            state_key = (
                ...
                if is_admin
                else "public_ranges_state_filter"
            )
        ...
        with col2:
            ...
            location_key = (
                ...
                if is_admin
                else "public_ranges_location_search"
            )
        ...
        if selected_state != "All States":
            filtered_ranges = [
                range_data
                for range_data in filtered_ranges
                if self._extract_address_components(range_data).get("state", "")
                == selected_state
            ]
        ...
        if not filtered_ranges:
            st.warning(
                "🔍 No ranges match your search criteria. Try adjusting your filters."
            )
            return {"action": None, "selected_indices": []}
            ...
            table_data.append(
                {
                    ...
                    "Location": range_data.get("display_name", "")[:85]
                    + ("..." if len(range_data.get("display_name", "")) > 85 else ""),
                    "Description": range_data.get("range_description", "")[:50]
                    + (
                        "..."
                        if len(range_data.get("range_description", "")) > 50
                        else ""
                    ),
        ...
        table_key = (
            ...
            if is_admin
            else "public_ranges_readonly_table_checkboxes"
        )
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Firing lat": st.column_config.TextColumn(
                    "Firing lat", width="small", disabled=True
                ),
                "Firing lon": st.column_config.TextColumn(
                    "Firing lon", width="small", disabled=True
                ),
                "Firing alt (m)": st.column_config.TextColumn(
                    "Firing alt (m)", width="small", disabled=True
                ),
                "Azimuth angle (°)": st.column_config.TextColumn(
                    "Azimuth angle (°)", width="small", disabled=True
                ),
                "Elevation angle (°)": st.column_config.TextColumn(
                    "Elevation angle (°)", width="small", disabled=True
                ),
                "Elevation change (m)": st.column_config.TextColumn(
                    "Elevation change (m)", width="small", disabled=True
                ),
                "County": st.column_config.TextColumn(
                    "County", width="medium", disabled=True
                ),
    ...
    def display_ranges_map(
        self, ranges: List[Dict[str, Any]], selected_indices: List[int] = None
    ) -> folium.Map:
