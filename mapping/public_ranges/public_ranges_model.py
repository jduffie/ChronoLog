...
class PublicRangesModel:
    ...
    def get_public_range_by_id(self, range_id: str, supabase_client) -> Dict[str, Any]:
        ...
        try:
            result = supabase_client.table("ranges").select("*").eq("id", range_id).execute()
    ...
    def search_public_ranges(self, search_term: str, supabase_client) -> List[Dict[str, Any]]:
        ...
        try:
            result = supabase_client.table("ranges").select(
                ...
                "start_altitude_m, end_altitude_m, distance_m, azimuth_deg, elevation_angle_deg, "
                "display_name, submitted_at, address_geojson"
            ).or_(f"range_name.ilike.%{search_term}%,range_description.ilike.%{search_term}%,display_name.ilike.%{search_term}%").order("submitted_at", desc=True).execute()
            return result.data if result.data else []
        except Exception as e:
            print(f"Error searching public ranges: {e}")
            return []

    def get_ranges_by_location(self, min_lat: float, max_lat: float, min_lon: float, max_lon: float, supabase_client) -> List[Dict[str, Any]]:
        ...
        try:
            result = supabase_client.table("ranges").select(
                ...
                "start_altitude_m, end_altitude_m, distance_m, azimuth_deg, elevation_angle_deg, "
                "display_name, submitted_at, address_geojson"
            ).gte("start_lat", min_lat).lte("start_lat", max_lat).gte("start_lon", min_lon).lte("start_lon", max_lon).order("submitted_at", desc=True).execute()
            return result.data if result.data else []
        except Exception as e:
            print(f"Error getting ranges by location: {e}")
            return []

    def get_ranges_by_distance(self, min_distance: float, max_distance: float, supabase_client) -> List[Dict[str, Any]]:
        ...
        try:
            result = supabase_client.table("ranges").select(
                ...
                "start_altitude_m, end_altitude_m, distance_m, azimuth_deg, elevation_angle_deg, "
                "display_name, submitted_at, address_geojson"
            ).gte("distance_m", min_distance).lte("distance_m", max_distance).order("distance_m", desc=False).execute()
            return result.data if result.data else []
        except Exception as e:
    ...
    def get_range_statistics(self, supabase_client) -> Dict[str, Any]:
        ...
        try:
            ...
            distances = [r['distance_m'] for r in result.data if r['distance_m'] is not None]
            if not distances:
                return {}
    ...
    def add_public_range(self, range_data: Dict[str, Any], supabase_client) -> bool:
        ...
        try:
            ...
            if result.data:
                print(f"Successfully added public range: {range_data.get('range_name', 'Unknown')}")
                return True
            else:
                print(f"Failed to add public range: {range_data.get('range_name', 'Unknown')}")
                return False
        except Exception as e:
            print(f"Error adding public range: {e}")
            return False

    def update_public_range(self, range_id: str, updates: Dict[str, Any], supabase_client) -> bool:
        """Update a public range."""
        try:
            result = supabase_client.table("ranges").update(updates).eq("id", range_id).execute()
            if result.data:
