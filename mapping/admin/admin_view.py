...
class AdminView:
    ...
    def display_submission_actions(self, submission: Dict[str, Any], review_reason: str) -> Optional[str]:
    ...
    def display_submissions_table(self, submissions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for submission in submissions:
            display_data.append({
                "ID": submission.get('id', 'N/A')[:8] + "...",  # Show first 8 chars of ID
        ...
        with col2:
            user_filter = st.text_input("Filter by User Email:", placeholder="Enter email to filter")
        
        with col3:
            range_filter = st.text_input("Filter by Range Name:", placeholder="Enter range name to filter")
        ...
        if user_filter:
            filtered_submissions = [s for s in filtered_submissions if user_filter.lower() in s.get('user_email', '').lower()]
        if range_filter:
            filtered_submissions = [s for s in filtered_submissions if range_filter.lower() in s.get('range_name', '').lower()]
        ...
        if filtered_display_data:
            ...
            # Show count
            st.markdown(f"**Showing {len(filtered_submissions)} of {len(submissions)} submissions**")
        else:
    ...
    def display_bulk_actions(self, submissions: List[Dict[str, Any]]) -> Optional[str]:
        ...
        submission_options = {i: f"{sub.get('range_name', f'Range {i+1}')} - {sub.get('user_email', 'Unknown')}" for i, sub in enumerate(submissions)}
        ...
        if selected_indices:
            ...
            with col1:
                if st.button("✅ Bulk Approve", disabled=not selected_indices, use_container_width=True, type="primary"):
                    return "bulk_approve"
            
            with col2:
                if st.button("❌ Bulk Deny", disabled=not selected_indices, use_container_width=True):
                    return "bulk_deny"
            
            with col3:
                if st.button("🔄 Bulk Reset", disabled=not selected_indices, use_container_width=True):
                    return "bulk_reset"
    ...
    def display_statistics(self, submissions: List[Dict[str, Any]]) -> None:
        ...
        with col2:
            st.metric("Pending Review", pending, delta=f"{(pending/total*100):.1f}%" if total > 0 else "0%")
        
        with col3:
            st.metric("Approved", approved, delta=f"{(approved/total*100):.1f}%" if total > 0 else "0%")
        
        with col4:
            st.metric("Denied", denied, delta=f"{(denied/total*100):.1f}%" if total > 0 else "0%")