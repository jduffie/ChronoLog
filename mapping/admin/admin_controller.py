...
class AdminController:
    ...
    def _handle_submission_action(
        self, submission: Dict[str, Any], action: str, review_reason: str
    ) -> None:
        ...
        try:
            ...
            if action == "approve":
                success = self.model.approve_submission(
                    submission["id"], review_reason.strip(), supabase
                )
                if success:
                    self.view.display_success_message(
                        "✅ Range approved and added to public ranges!"
                    )
                    st.rerun()
                else:
                    self.view.display_error_message(
                        "❌ Failed to approve range. Please try again."
                    )

            elif action == "deny":
                success = self.model.deny_submission(
                    submission["id"], review_reason.strip(), supabase
                )
                if success:
                    self.view.display_success_message(
                        "✅ Range denied with feedback sent to user."
                    )
                    st.rerun()
                else:
                    self.view.display_error_message(
                        "❌ Failed to deny range. Please try again."
                    )
                ...
                if success:
                    ...
                    self.view.display_error_message(
                        "❌ Failed to reset status. Please try again."
                    )
    ...
    def _handle_bulk_actions(
        ...
        if not selected_indices:
            self.view.display_warning_message(
                "No submissions selected for bulk action."
            )
            return
        ...
        if st.button(
            ...
            try:
                ...
                for idx in selected_indices:
                    if idx < len(submissions):
                        ...
                        if action == "bulk_approve":
                            if self.model.approve_submission(
                                submission["id"], bulk_reason.strip(), supabase
                            ):
                                success_count += 1
                        elif action == "bulk_deny":
                            if self.model.deny_submission(
                                submission["id"], bulk_reason.strip(), supabase
                            ):
                                success_count += 1
                        elif action == "bulk_reset":
                            if self.model.reset_submission_status(
                ...
                if success_count > 0:
                    self.view.display_success_message(
                        f"✅ Successfully processed {success_count} submissions!"
                    )
                    # Clear selection
                ...
                self.view.display_error_message(
                    f"❌ Error processing bulk action: {str(e)}"
                )
    ...
    def run(self) -> None:
        ...
        SessionStateManager.set_current_page("admin")
        ...
        try:
            ...
            tab1, tab2, tab3 = st.tabs(
                ["Pending Review", "All Submissions", "Statistics"]
            )
            ...
            with tab1:
                ...
                if not pending_submissions:
                    ...
                    for submission in pending_submissions:
                        review_reason = self.view.display_submission_details(
                            submission, 0
                        )
                        action = self.view.display_submission_actions(
                            submission, review_reason
                        )

                        if action:
                            self._handle_submission_action(
                                submission, action, review_reason
                            )
            ...
            with tab2:
                ...
                if all_submissions:
                    ...
                    bulk_action = self.view.display_bulk_actions(
                        table_result.get("filtered_submissions", [])
                    )
    ...
    def get_pending_submissions(self):
        ...
        try:
            ...
            self.view.display_error_message(
                f"Error fetching pending submissions: {str(e)}"
            )
            return []
