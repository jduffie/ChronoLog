...
class ChronographService:
    ...
    def get_sessions_for_user(self, user_email: str) -> List[ChronographSession]:
        ...
        try:
            response = self.supabase.table("chrono_sessions").select("*").eq("user_email", user_email).order("datetime_local", desc=True).execute()
    ...
    def get_session_by_id(self, session_id: str, user_email: str) -> Optional[ChronographSession]:
        """Get a specific chronograph session by ID"""
        try:
            response = self.supabase.table("chrono_sessions").select("*").eq("id", session_id).eq("user_email", user_email).single().execute()
    ...
    def get_sessions_filtered(self, user_email: str, bullet_type: Optional[str] = None, 
                            start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[ChronographSession]:
        """Get filtered chronograph sessions"""
        try:
            query = self.supabase.table("chrono_sessions").select("*").eq("user_email", user_email)
    ...
    def get_unique_bullet_types(self, user_email: str) -> List[str]:
        ...
        try:
            response = self.supabase.table("chrono_sessions").select("bullet_type").eq("user_email", user_email).execute()
            
            if not response.data:
                return []
            
            bullet_types = list(set([record["bullet_type"] for record in response.data if record.get("bullet_type")]))
            return sorted(bullet_types)
        
        except Exception as e:
            raise Exception(f"Error fetching bullet types: {str(e)}")
    
    def session_exists(self, user_email: str, tab_name: str, datetime_local: str) -> bool:
        """Check if a session already exists"""
        try:
            response = self.supabase.table("chrono_sessions").select("id").eq("user_email", user_email).eq("tab_name", tab_name).eq("datetime_local", datetime_local).execute()
    ...
    def update_session_stats(self, session_id: str, stats: dict) -> None:
        ...
        try:
            self.supabase.table("chrono_sessions").update(stats).eq("id", session_id).execute()
    ...
    def get_measurements_for_stats(self, user_email: str, tab_name: str, session_date: str) -> List[float]:
        ...
        try:
            ...
            speeds = [record["speed_fps"] for record in response.data if record.get("speed_fps") is not None]
            return speeds
