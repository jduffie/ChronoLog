...
class ChronographDeviceAdapter(ABC):
    ...
    @abstractmethod
    def ingest_data(self, data_source: Any, **kwargs) -> ChronographIngestResult:
        """
...
class GarminExcelAdapter(ChronographDeviceAdapter):
    ...
    def ingest_data(self, excel_file: pd.ExcelFile, **kwargs) -> ChronographIngestResult:
        ...
        for _, row in data.iterrows():
            ...
            measurement = self._create_measurement_entity(row, column_units, session_timestamp)
            if measurement:
        ...
        for header in data_columns:
            if header in unit_mapping:
                ...
                if mapping['imperial'] in header:
                    column_units[header] = self._map_imperial_unit(mapping['imperial'])
                elif mapping['metric'] in header:
                    column_units[header] = self._map_metric_unit(mapping['metric'])
                else:
    ...
    def _create_measurement_entity(self, row, column_units: dict, session_timestamp: str) -> Optional[ChronographMeasurementEntity]:
...
class ChronographDeviceFactory:
    ...
    @staticmethod
    def create_adapter(device_type: str, unit_mapping_service) -> ChronographDeviceAdapter:
        ...
        if device_type.lower() == "garmin_excel":
            ...
            raise ValueError(f"Unsupported chronograph device type: {device_type}")
    ...
    def detect_device_type(file_name: str, data_source: Any) -> str:
        ...
        if file_name.lower().endswith('.xlsx') and hasattr(data_source, 'iloc'):
            ...
            if len(data_source) > 1 and data_source.iloc[0, 0] and ',' in str(data_source.iloc[0, 0]):
                return "garmin_excel"
