...
def render_chronograph_import_tab(user, supabase, bucket):
    ...
    if wizard_state["step"] == "source_selection":
        ...
        if source_choice == "Select existing source":
            if not sources:
                st.warning(
                    "No existing chronograph sources found. Please create a new source.")
            else:
                ...
                if st.form_submit_button(
                    ...
                    if not name.strip():
                        ...
                        try:
                            ...
                            existing = chrono_service.get_source_by_name(
                                user["id"], name.strip())
                            if existing:
                                st.error(
                                    f"A chronograph source named '{name}' already exists!")
                            else:
                                source_data = {
                                    "user_id": user["id"],
                                    "name": name.strip(),
                                ...
                                source_id = chrono_service.create_source(
                                    source_data)
                                wizard_state["selected_source_id"] = source_id
                                wizard_state["step"] = "file_upload"
                                st.success(
                                    f"Chronograph source '{name}' created successfully!")
                                st.rerun()
    ...
    # Step 2: File Upload
        ...
        selected_source = chrono_service.get_source_by_id(
