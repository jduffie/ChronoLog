...
class TestWeatherSource(unittest.TestCase):
    ...
    def test_weather_source_display_methods(self):
        ...
        self.assertEqual(
            source.short_display(), "Test Kestrel - Kestrel 5700 Elite (S/N: K123456)"
        )
...
class TestWeatherServiceAdvanced(unittest.TestCase):
    ...
    def test_weather_source_crud_operations(self):
        ...
        mock_response.data = [{"id": self.test_source_id, **self.sample_source_data}]
        ...
        source = self.weather_service.get_source_by_id(self.test_source_id, self.test_user_id)
        self.assertIsNotNone(source)
        self.assertEqual(source.id, self.test_source_id)
        
        # Test get by name
        source = self.weather_service.get_source_by_name(self.test_user_id, "Test Kestrel")
        self.assertIsNotNone(source)
        
        # Test update
        updates = {"name": "Updated Kestrel"}
        self.weather_service.update_source(self.test_source_id, self.test_user_id, updates)
        self.mock_supabase.table.return_value.update.assert_called()
        
        # Test delete
        self.weather_service.delete_source(self.test_source_id, self.test_user_id)
    ...
    def test_weather_measurement_crud_operations(self):
        ...
        mock_response = MagicMock()
        mock_response.data = [{"id": "measurement-123", **self.sample_measurement_data}]
        self.mock_supabase.table.return_value.insert.return_value.execute.return_value = mock_response
        
        measurement_id = self.weather_service.create_measurement(self.sample_measurement_data)
        ...
        created_ids = self.weather_service.create_measurements_batch(batch_data)
        ...
        for method_name, error_msg in error_scenarios:
            with self.subTest(method=method_name):
                ...
                if method_name == "get_sources_for_user":
                    self.mock_supabase.table.return_value.select.return_value.eq.return_value.order.return_value.execute.side_effect = Exception(error_msg)
                elif method_name == "create_source":
                    self.mock_supabase.table.return_value.insert.return_value.execute.side_effect = Exception(error_msg)
                elif method_name == "get_measurements_for_source":
                    self.mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.order.return_value.execute.side_effect = Exception(error_msg)
                ...
                with self.assertRaises(Exception) as context:
                    if method_name == "get_sources_for_user":
                        self.weather_service.get_sources_for_user(self.test_user_id)
                    elif method_name == "create_source":
                        self.weather_service.create_source(self.sample_source_data)
                    elif method_name == "get_measurements_for_source":
                        self.weather_service.get_measurements_for_source(self.test_source_id, self.test_user_id)
                    elif method_name == "create_measurement":
                        self.weather_service.create_measurement(self.sample_measurement_data)
    ...
    def test_bulk_operations_performance(self):
        ...
        created_ids = self.weather_service.create_measurements_batch(large_batch)
        self.assertEqual(len(created_ids), 100)
        
        # Verify single batch call was made
        self.mock_supabase.table.return_value.insert.assert_called_once_with(large_batch)
...
class TestWeatherModelsAdvanced(unittest.TestCase):
    ...
    def test_weather_source_edge_cases(self):
        ...
        self.assertEqual(minimal_source.short_display(), "Minimal Source - Unknown Device")
    ...
    def test_weather_measurement_comprehensive_display(self):
        ...
        self.assertEqual(full_measurement.density_altitude_display(), "2100 ft")
        ...
        self.assertEqual(measurement_mag_only.wind_direction_display(), "275° (mag)")
    ...
    def test_weather_measurement_data_detection(self):
        ...
        for combo in location_data_combinations:
            ...
            self.assertEqual(measurement.has_location_data(), combo["expected"])
    ...
    def test_weather_models_from_supabase_records(self):
        ...
        measurements = WeatherMeasurement.from_supabase_records(measurement_records)
        self.assertEqual(len(measurements), 5)
...
class TestWeatherIntegrationAdvanced(unittest.TestCase):
    ...
    def test_complete_weather_workflow_integration(self):
        ...
        batch_response.data = [{"id": f"int-measurement-{i}"} for i in range(10)]
        self.mock_supabase.table.return_value.insert.return_value.execute.return_value = batch_response
        
        measurement_ids = self.weather_service.create_measurements_batch(measurements)
        ...
        retrieved_measurements = self.weather_service.get_measurements_for_source(source_id, self.test_user_id)
        ...
        for scenario in device_scenarios:
            with self.subTest(scenario=scenario["expected_action"]):
                ...
                if scenario["expected_action"] == "find_existing":
                    ...
                    # Setup mock chain for serial search -> name search -> create
    ...
    def test_data_consistency_across_operations(self):
        ...
        self.mock_supabase.table.return_value.update.assert_called_with(expected_update)
        ...
        measurement_response.data = [{"id": "consistency-measurement", **measurement_data}]
        self.mock_supabase.table.return_value.insert.return_value.execute.return_value = measurement_response
        
        measurement_id = self.weather_service.create_measurement(measurement_data)
    ...
    def test_error_recovery_and_rollback_simulation(self):
        ...
        large_batch = [{"user_id": self.test_user_id, "weather_source_id": "source-1", "measurement_timestamp": f"2023-12-01T{i:02d}:00:00Z"} for i in range(50)]
        
        # Simulate partial failure in batch insert
        self.mock_supabase.table.return_value.insert.return_value.execute.side_effect = Exception("Batch insert failed at record 25")
        ...
        self.mock_supabase.reset_mock()
        self.mock_supabase.table.return_value.insert.return_value.execute.side_effect = Exception("Source creation failed")
        
        with self.assertRaises(Exception) as context:
            self.weather_service.create_source({"user_id": self.test_user_id, "name": "Fail Source"})
