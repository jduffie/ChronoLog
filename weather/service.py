...
class WeatherService:
    ...
    def get_sources_for_user(self, user_email: str) -> List[WeatherSource]:
        ...
        try:
            response = self.supabase.table("weather_source").select("*").eq("user_email", user_email).order("name").execute()
    ...
    def get_source_by_id(self, source_id: str, user_email: str) -> Optional[WeatherSource]:
        """Get a specific weather source by ID"""
        try:
            response = self.supabase.table("weather_source").select("*").eq("id", source_id).eq("user_email", user_email).single().execute()
    ...
    def get_source_by_name(self, user_email: str, name: str) -> Optional[WeatherSource]:
        ...
        try:
            response = self.supabase.table("weather_source").select("*").eq("user_email", user_email).eq("name", name).single().execute()
    ...
    def create_source(self, source_data: dict) -> str:
        ...
        try:
            response = self.supabase.table("weather_source").insert(source_data).execute()
    ...
    def update_source(self, source_id: str, user_email: str, updates: dict) -> None:
        ...
        try:
            ...
            self.supabase.table("weather_source").update(updates).eq("id", source_id).eq("user_email", user_email).execute()
    ...
    def delete_source(self, source_id: str, user_email: str) -> None:
        ...
        try:
            self.supabase.table("weather_source").delete().eq("id", source_id).eq("user_email", user_email).execute()
        
        except Exception as e:
            raise Exception(f"Error deleting weather source: {str(e)}")
    
    # Weather Measurements
    def get_measurements_for_source(self, source_id: str, user_email: str) -> List[WeatherMeasurement]:
        """Get all measurements for a specific weather source"""
        try:
            response = self.supabase.table("weather_measurements").select("*").eq("weather_source_id", source_id).eq("user_email", user_email).order("measurement_timestamp").execute()
    ...
    def get_all_measurements_for_user(self, user_email: str) -> List[WeatherMeasurement]:
        """Get all weather measurements for a user"""
        try:
            response = self.supabase.table("weather_measurements").select("*").eq("user_email", user_email).order("measurement_timestamp", desc=True).execute()
    ...
    def get_measurements_filtered(self, user_email: str, source_id: Optional[str] = None, 
                                start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[WeatherMeasurement]:
        """Get filtered weather measurements"""
        try:
            query = self.supabase.table("weather_measurements").select("*").eq("user_email", user_email)
    ...
    def create_measurement(self, measurement_data: dict) -> str:
        ...
        try:
            response = self.supabase.table("weather_measurements").insert(measurement_data).execute()
    ...
    def measurement_exists(self, user_email: str, source_id: str, measurement_timestamp: str) -> bool:
        """Check if a measurement already exists"""
        try:
            response = self.supabase.table("weather_measurements").select("id").eq("user_email", user_email).eq("weather_source_id", source_id).eq("measurement_timestamp", measurement_timestamp).execute()
    ...
    def update_source_with_device_info(self, source_id: str, user_email: str, device_name: str, device_model: str, serial_number: str) -> None:
    ...
    def create_or_get_source_from_device_info(self, user_email: str, device_name: str, device_model: str, serial_number: str) -> str:
        """Create or get existing weather source from device information"""
        try:
            # First try to find existing source by serial number
            if serial_number:
                response = self.supabase.table("weather_source").select("*").eq("user_email", user_email).eq("serial_number", serial_number).execute()
                if response.data:
                    return response.data[0]["id"]
