...
def render_weather_import_tab(user, supabase, bucket):
    ...
    if not sources:
        st.warning("⚠️ No weather sources found. Please go to the **Sources** tab to create a weather source first.")
        return
    ...
    source_options = [""] + [source.name for source in sources]  # Empty string for initial undefined state
    ...
    if selected_source_name:
        ...
        st.success(f"📡 Selected: {selected_source.name} - {selected_source.device_display()}")
...
def render_file_upload(user, supabase, bucket, weather_service, selected_meter_id):
    ...
    uploaded_file = st.file_uploader("Upload Kestrel CSV File", type=["csv"], key="weather_upload")
    if uploaded_file:
        try:
            ...
            try:
                ...
                if "already exists" in str(upload_error) or "409" in str(upload_error):
                    ...
                    st.warning(f"⚠️ Weather file '{uploaded_file.name}' already exists in storage.")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("🔄 Overwrite File", key="overwrite_file", type="primary"):
                            try:
                                ...
                                st.error(f"❌ Error overwriting file: {overwrite_error}")
                                return
                    
                    with col2:
                        if st.button("❌ Cancel Import", key="cancel_import"):
                            st.info("Import cancelled. You can go to the 'My Files' tab to manage existing files.")
                            return
                    
                    # If neither button was clicked, stop processing
                    if 'overwrite_file' not in st.session_state and 'cancel_import' not in st.session_state:
                        return
                else:
            ...
            try:
                weather_service.update_source_with_device_info(
                    selected_meter_id, user["email"], device_name, device_model, serial_number
                )
            ...
            for row_data in data_rows:
                try:
                    ...
                    if weather_service.measurement_exists(user["email"], source_id, measurement_timestamp):
                        skipped_measurements += 1
                        continue
            ...
            try:
                ...
                if source:
                    st.info(f"📱 Weather Source: {source.display_name()} - {source.device_display()}")
                else:
