    ...
    try:
        ...
        for source in sources:
            ...
            if source_measurements:
                ...
                date_range = f"{first_date} to {last_date}" if first_date != last_date else str(first_date)
                
                # Get temperature stats if available
                temps = [m.temperature_f for m in source_measurements if m.temperature_f is not None]
                temp_range = f"{min(temps):.1f}°F - {max(temps):.1f}°F" if temps else "N/A"
                
                # Get humidity stats if available
                humidity = [m.relative_humidity_pct for m in source_measurements if m.relative_humidity_pct is not None]
                humidity_range = f"{min(humidity):.1f}% - {max(humidity):.1f}%" if humidity else "N/A"
        ...
        with col1:
            source_options = ["All"] + [f"{source.display_name()}" for source in sources]
        ...
        if selected_source != "All":
            ...
            if selected_source_obj:
                filtered_measurements = [m for m in filtered_measurements if m.weather_source_id == selected_source_obj.id]
        
        if date_range:
            if len(date_range) == 2:
                start_date, end_date = date_range
                filtered_measurements = [m for m in filtered_measurements 
                                       if start_date <= pd.to_datetime(m.measurement_timestamp).date() <= end_date]
        ...
        if filtered_measurements:
            ...
            for measurement in filtered_measurements:
                ...
                date_key = pd.to_datetime(measurement.measurement_timestamp).strftime('%Y-%m-%d')
            ...
            for source_id, dates_data in measurements_by_source.items():
                ...
                if source_obj:
                    total_measurements = sum(len(date_measurements) for date_measurements in dates_data.values())
                    
                    with st.expander(f"📡 {source_obj.display_name()} ({total_measurements} measurements)", expanded=True):
                        st.write(f"**Device:** {source_obj.device_display()}")
                        
                        # Display each date group within this source
                        for date_str, date_measurements in sorted(dates_data.items(), reverse=True):
                            with st.expander(f"📅 {date_str} ({len(date_measurements)} measurements)", expanded=False):
                                ...
                                if available_columns:
                                    ...
                                    if st.button(f"View Detailed Analysis", key=f"view_weather_{source_obj.id}_{date_str}"):
                                        st.session_state['selected_weather_source_id'] = source_obj.id
                                        st.session_state['selected_weather_date'] = date_str
                                        st.info("Date and source selected! Go to the 'View Log' tab to see detailed weather analysis.")
                                else:
