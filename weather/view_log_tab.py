...
def render_weather_view_log_tab(user, supabase):
    ...
    try:
        ...
        selected_option = st.selectbox(
            ...
            index=(
                unique_options.index(default_selection)
                if default_selection and default_selection in unique_options
                else 0
            ),
        )
        ...
        selected_measurements = [
            ...
            if (
                m.weather_source_id == selection_info["source_id"]
                and pd.to_datetime(m.measurement_timestamp).strftime("%Y-%m-%d")
                == selection_info["date"]
            )
        ]
        ...
        if (
            "relative_humidity_pct" in df.columns
            and not df["relative_humidity_pct"].isna().all()
        ):
        ...
        chart_tab1, chart_tab2, chart_tab3 = st.tabs(
            ["Temperature & Humidity", "Pressure & Wind", "All Parameters"]
        )
        ...
        with chart_tab3:
            ...
            if (
                ...
                )  # Already 0-100, just scale to 0-1
                ax.plot(
                    df["timestamp"],
                    humidity_norm,
                    "b-",
