...
def render_weather_sources_tab(user, supabase):
    ...
    try:
        ...
        with manage_tab:
            ...
            if not sources:
                st.info("No weather sources found. Use the 'Add Source' tab to create your first weather source.")
            ...
            for source in sources:
                ...
                source_measurements = [m for m in measurements if m.weather_source_id == source.id]
                ...
                with st.expander(f"📡 {source.display_name()}", expanded=False):
                    ...
                    with col1:
                        ...
                        if source.created_at:
                            st.write(f"• **Created:** {pd.to_datetime(source.created_at).strftime('%Y-%m-%d %H:%M')}")
                    ...
                    with col2:
                        ...
                        if source_measurements:
                            ...
                            # Date range
                            timestamps = [pd.to_datetime(m.measurement_timestamp) for m in source_measurements]
                            ...
                            # Last upload
                            last_upload = max([pd.to_datetime(m.uploaded_at) for m in source_measurements]).date()
                    ...
                    with button_col2:
                        if source_measurements:
                            if st.button("View Data", key=f"view_{source.id}"):
                                ...
                                if source_measurements:
                                    latest_date = max([pd.to_datetime(m.measurement_timestamp) for m in source_measurements]).strftime('%Y-%m-%d')
                                    st.session_state['selected_weather_source_id'] = source.id
                                    st.session_state['selected_weather_date'] = latest_date
                                    st.info("Source selected! Go to the 'View Log' tab to see detailed weather analysis.")
                    
                    with button_col3:
                        if st.button("Delete", key=f"delete_{source.id}", type="secondary"):
                    ...
                    if st.session_state.get(f'edit_source_{source.id}', False):
                        ...
                        with st.form(f"edit_form_{source.id}"):
                            ...
                            new_device_name = st.text_input("Device Name", value=source.device_name or "")
                            new_make = st.text_input("Make/Manufacturer", value=source.make or "")
                            new_model = st.text_input("Model", value=source.model or "")
                            new_serial = st.text_input("Serial Number", value=source.serial_number or "")
                            ...
                            with col1:
                                if st.form_submit_button("Save Changes"):
                                    try:
                                        updates = {
                                            "name": new_name,
                                            "device_name": new_device_name if new_device_name else None,
                                            "make": new_make if new_make else None,
                                            "model": new_model if new_model else None,
                                            "serial_number": new_serial if new_serial else None
                                        }
                                        weather_service.update_source(source.id, user["email"], updates)
                                        st.success("Weather source updated successfully!")
                    ...
                    if st.session_state.get(f'confirm_delete_{source.id}', False):
                        ...
                        if source_measurements:
                            st.warning(f"This will also delete {len(source_measurements)} weather measurements!")
                        
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("Yes, Delete", key=f"confirm_yes_{source.id}", type="primary"):
                                try:
                                    weather_service.delete_source(source.id, user["email"])
                                    st.success(f"Weather source '{source.name}' deleted successfully!")
        ...
        with add_tab:
                ...
                if st.form_submit_button("Save Weather Source", type="primary"):
                    if not name.strip():
                        ...
                        try:
                            ...
                            existing = weather_service.get_source_by_name(user["email"], name.strip())
                            if existing:
            ...
            st.write("---")
            st.info("💡 **Tip:** Create weather sources here first, then go to the Import tab to upload Kestrel CSV files. Device details will be automatically extracted from your CSV files.")
