...
def first_match(text: str, patterns: list[re.Pattern], group: int = 1) -> Optional[str]:
    for p in patterns:
        m = p.search(text)
        if m:
...
def first_number_around(label: str, full_text: str) -> Optional[float]:
    ...
    m = re.search(r'(?<![\w\.])(\d{1,2}(?:\.\d{1,3})?|\.\d{2,3})(?![\w\.])', window)
    if not m:
        return None
...
COLUMNS = [
    "name", "bore_diameter_land_mm", "saami_name", "saami_category",
    "saami_case_length_in", "saami_case_length_mm",
    "saami_cartridge_overall_length_in", "saami_cartridge_overall_length_mm",
    "saami_bore_diameter_in", "saami_bore_diameter_mm",
    "saami_groove_diameter_in", "saami_groove_diameter_mm",
    "saami_bullet_diameter_in", "saami_bullet_diameter_mm",
    "saami_case_head_diameter_in", "saami_case_head_diameter_mm",
    "saami_neck_diameter_in", "saami_neck_diameter_mm",
    "saami_shoulder_diameter_in", "saami_shoulder_diameter_mm",
    "saami_rim_diameter_in", "saami_rim_diameter_mm",
    "saami_rim_thickness_in", "saami_rim_thickness_mm",
    "saami_max_avg_pressure_psi", "saami_max_avg_pressure_cup",
    "saami_max_avg_pressure_bar", "saami_max_avg_pressure_mpa",
    "saami_test_barrel_length_in", "saami_test_barrel_length_mm",
    "saami_reference_velocity_fps", "saami_reference_velocity_ms",
    "saami_reference_bullet_weight_gr", "saami_reference_bullet_weight_g",
    "saami_headspace_go_gauge_in", "saami_headspace_go_gauge_mm",
    "saami_headspace_no_go_gauge_in", "saami_headspace_no_go_gauge_mm",
    "saami_chamber_throat_diameter_in", "saami_chamber_throat_diameter_mm",
    "saami_chamber_throat_length_in", "saami_chamber_throat_length_mm",
    "saami_rifling_twist_rate", "saami_rifling_groove_count",
    "saami_standard_reference", "saami_approved_date", "saami_last_revised_date",
    "saami_source_url"
]
...
def parse_saami_plate(image_path: str) -> Dict[str, Any]:
    ...
    # Name / category
    # Header like: "6.5 CREEDMOOR [6.5 CM]" and "CARTRIDGE ...", "CHAMBER ...", or a category string.
    name = first_match(text, [re.compile(r'^\s*([0-9.]+\s*mm?\s*Creedmoor)', re.IGNORECASE | re.MULTILINE),
                              re.compile(r'^\s*([0-9.]+\s*Creedmoor)', re.IGNORECASE | re.MULTILINE)])
    row.name = row.saami_name = name
    cat = first_match(text, [re.compile(r'(CENTERFIRE\s+RIFLE)', re.IGNORECASE),
                             re.compile(r'(RIMFIRE|CENTERFIRE)\s+(PISTOL|RIFLE)', re.IGNORECASE)])
    row.saami_category = cat.upper() if cat else None

    # Dates (Issued/Revised)
    row.saami_approved_date = first_match(text, [re.compile(r'ISSUED:\s*([0-9/.-]+)', re.IGNORECASE)])
    row.saami_last_revised_date = first_match(text, [re.compile(r'REVISED:\s*([0-9/.-]+)', re.IGNORECASE)])
    ...
    bullet_diam_in = first_match(text, [
        re.compile(r'BULLET.*?\b(\.\d{3})\b.*?\((\d{1,2}\.\d{1,3})\)', re.IGNORECASE),
        re.compile(r'BULLET.*?\((\d{1,2}\.\d{1,3})\)', re.IGNORECASE)  # sometimes only metric shows nearby
    ])
    # If bullet diameter inches not explicit, use groove dia inches (common equal)
    ...
    # Assign diameters
    row.saami_bore_diameter_mm = parse_float(bore_mm) or first_number_around("Bore Dia", text)
    ...
    if row.saami_bore_diameter_in is None and row.saami_bore_diameter_mm:
        ...
        row.saami_bore_diameter_in = round(row.saami_bore_diameter_mm / 25.4, 3)
    if row.saami_groove_diameter_in is None and row.saami_groove_diameter_mm:
        row.saami_groove_diameter_in = round(row.saami_groove_diameter_mm / 25.4, 3)
    ...
    # set it from groove metric instead; otherwise we keep it equal to Bore Dia.
    row.bore_diameter_land_mm = row.saami_bore_diameter_mm  # change to row.saami_groove_diameter_mm if desired
    ...
    row.saami_case_length_mm = parse_float(case_mm) or inch_to_mm(row.saami_case_length_in)

    # COAL (Cartridge Overall Length) — lines often show “2.700 MIN – 2.825 MAX”
    ...
    coal_mm = first_match(text, [
        re.compile(r'Overall\s*Length.*?\(\s*([0-9.]{2,6})\s*\)\s*MAX', re.IGNORECASE)
    ])
    row.saami_cartridge_overall_length_in = parse_float(coal_in)
    row.saami_cartridge_overall_length_mm = parse_float(coal_mm) or inch_to_mm(row.saami_cartridge_overall_length_in)
    ...
    def dim_pair(label: str) -> tuple[Optional[float], Optional[float]]:
        ...
        inch = first_match(text, [re.compile(rf'(\.\d{{3}})\s*\(\s*{mm}\s*\)\s*{re.escape(label)}', re.IGNORECASE)]) if mm else None
        inch_val = parse_float(inch) if inch else None
        if inch_val is None and mm:
            inch_val = round(mm / 25.4, 3)
        return inch_val, mm

    row.saami_case_head_diameter_in, row.saami_case_head_diameter_mm = dim_pair("BODY DIA")  # many plates show body/rim; adjust if needed
    # More targeted patterns:
    row.saami_neck_diameter_mm = first_number_around("NECK", text)
    row.saami_neck_diameter_in = round(row.saami_neck_diameter_mm / 25.4, 3) if row.saami_neck_diameter_mm else None

    row.saami_shoulder_diameter_mm = first_number_around("SHOULDER", text)
    row.saami_shoulder_diameter_in = round(row.saami_shoulder_diameter_mm / 25.4, 3) if row.saami_shoulder_diameter_mm else None

    row.saami_rim_diameter_mm = first_number_around("RIM", text)
    row.saami_rim_diameter_in = round(row.saami_rim_diameter_mm / 25.4, 3) if row.saami_rim_diameter_mm else None
    ...
    # MAP (pressure)
    psi = first_match(text, [
        re.compile(r'(?:(?:SAAMI\s*)?(?:Max(?:imum)?\s*Average\s*Pressure|MAP))\s*([0-9,]{4,6})\s*psi', re.IGNORECASE),
        re.compile(r'\b([0-9,]{4,6})\s*psi\b', re.IGNORECASE),
    ])
    row.saami_max_avg_pressure_psi = int(psi.replace(',', '')) if psi else None

    bar = first_match(text, [re.compile(r'\b([0-9]{3,5})\s*bar\b', re.IGNORECASE)])
    ...
    if row.saami_max_avg_pressure_bar and not row.saami_max_avg_pressure_mpa:
        row.saami_max_avg_pressure_mpa = round(row.saami_max_avg_pressure_bar / 10.0, 2)

    # Headspace gauges
    row.saami_headspace_go_gauge_in = parse_float(first_match(text, [re.compile(r'\bGO\b.*?([0-9.]{1,6})\s*in', re.IGNORECASE)])) \
                                      or first_number_around("GO", text)
    row.saami_headspace_go_gauge_mm = inch_to_mm(row.saami_headspace_go_gauge_in) if row.saami_headspace_go_gauge_in else None

    row.saami_headspace_no_go_gauge_in = parse_float(first_match(text, [re.compile(r'NO-?GO.*?([0-9.]{1,6})\s*in', re.IGNORECASE)])) \
                                         or first_number_around("NO GO", text) or first_number_around("NO-GO", text)
    row.saami_headspace_no_go_gauge_mm = inch_to_mm(row.saami_headspace_no_go_gauge_in) if row.saami_headspace_no_go_gauge_in else None

    # Throat (often absent)
    row.saami_chamber_throat_diameter_mm = first_number_around("THROAT", text)
    row.saami_chamber_throat_diameter_in = round(row.saami_chamber_throat_diameter_mm / 25.4, 3) if row.saami_chamber_throat_diameter_mm else None
    ...
    if row.saami_case_head_diameter_mm and not row.saami_case_head_diameter_in:
        row.saami_case_head_diameter_in = round(row.saami_case_head_diameter_mm / 25.4, 3)
