...
class UserView:
    ...
    def display_profile_setup_form(self, user: Dict[str, Any], existing_profile: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """Display profile setup form and return form data if submitted."""
        st.title("Complete Your Profile")
        st.markdown("Please provide the following information to complete your account setup:")

        # Pre-fill with existing data if available
        default_username = existing_profile.get("username", "") if existing_profile else ""
        default_state = existing_profile.get("state", "") if existing_profile else ""
        default_country = existing_profile.get("country", "United States") if existing_profile else "United States"
        default_units = existing_profile.get("unit_system", "Imperial") if existing_profile else "Imperial"
        ...
        with st.form("profile_setup"):
            ...
            countries = [
                ...
                "Norway", "Sweden", "Denmark", "Finland", "Austria", "Belgium",
    ...
    def display_user_management_admin(self, users: list[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Username": st.column_config.TextColumn("Username", width="medium", disabled=True),
                ...
                "Unit System": st.column_config.TextColumn("Units", width="small", disabled=True),
                "Created": st.column_config.TextColumn("Created", width="small", disabled=True),
                "Profile Complete": st.column_config.TextColumn("Complete", width="small", disabled=True)
            },
            key="users_table"
        )
        ...
        with col2:
            if st.button("🗑️ Delete Selected", disabled=not selected_indices, type="secondary"):
    ...
    def display_delete_confirmation(self, users: list[Dict[str, Any]], selected_indices: list[int]) -> Optional[str]:
        ...
        st.warning(f"⚠️ Are you sure you want to delete the following {len(selected_users)} user(s)?")
