    ...
    try:
            ...
            for file_item in root_files:
                if file_item.get("name"):
                    ...
                    if "." not in file_item["name"]:
                        ...
                        try:
                            subdir_files = supabase.storage.from_(bucket).list(
                                subdir_path
                            )
        ...
        if file_type_filter:
            ...
            for directory, files in file_tree.items():
                if directory.lower() == file_type_filter.lower() or (
                    file_type_filter.lower() == "kestrel"
                    and directory.lower() == "weather"
                ):
        ...
        for directory, files in file_tree.items():
            ...
            with st.expander(
                f"{directory_icon} **{directory.upper()}** ({len(files)} files)",
                expanded=True,
            ):
                ...
                if files:
                    ...
                    for file_info in sorted(
                        ...
                        if st.session_state.get(
                            ...
                            with confirm_col2:
                                if st.button(
                                    "❌ Cancel", key=f"confirm_no_{full_path}"
                                ):
                                    del st.session_state[f"confirm_delete_{full_path}"]
                                    st.rerun()
...
def get_file_type(filename, metadata):
    ...
    if (
        "metadata" in metadata
        and metadata["metadata"]
        and "mimetype" in metadata["metadata"]
    ):
