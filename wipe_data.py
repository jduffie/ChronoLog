...
def wipe_table(supabase, table_name):
    try:
        ...
        try:
            ...
            while True:
                # Get a batch of IDs
                batch_response = supabase.table(table_name).select("id").limit(batch_size).execute()
                ...
                print(f"  📝 Deleted {deleted_total}/{record_count} records from {table_name}")
            
            print(f"  ✅ Deleted all records from {table_name} ({deleted_total} records removed)")
