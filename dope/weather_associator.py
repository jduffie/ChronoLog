...
class WeatherSessionAssociator:
    ...
    def get_chrono_session_time_window(self, user_id: str, chrono_session_id: str, buffer_minutes: int = 30) -> Optional[Tuple[datetime, datetime]]:
        """
        ...
        try:
            ...
            raise Exception(f"Error getting chrono session time window: {str(e)}")
    
    def get_weather_sources_with_measurements(self, user_id: str, start_time: datetime, end_time: datetime) -> List[Dict]:
        """
        ...
        try:
            ...
            weather_sources = self.weather_service.get_sources_for_user(user_id)
            ...
            raise Exception(f"Error getting weather sources with measurements: {str(e)}")
    
    def get_weather_measurements_for_window(self, user_id: str, weather_source_id: str, start_time: datetime, end_time: datetime) -> List[Dict]:
        """
    ...
    def calculate_median_weather_values(self, measurements: List[Dict]) -> Dict:
        """
        ...
        weather_fields = [
            'temperature_f', 'relative_humidity_pct', 'barometric_pressure_inhg',
            'altitude_ft', 'wind_speed_mph', 'density_altitude_ft'
        ]
    ...
    def find_closest_weather_measurement(self, shot_timestamp: datetime, measurements: List[Dict]) -> Optional[str]:
        """
        ...
        for measurement in measurements:
            ...
            time_diff = abs((shot_timestamp - measurement_time).total_seconds())
    ...
    def associate_weather_with_dope_session(self, user_id: str, dope_session_id: str, weather_source_id: str, start_time: datetime, end_time: datetime) -> Dict:
        """
        ...
        try:
            ...
            if not weather_measurements:
                return {"error": "No weather measurements found for time window"}
            
            # Calculate median weather values
            median_weather = self.calculate_median_weather_values(weather_measurements)
            ...
            raise Exception(f"Error associating weather with DOPE session: {str(e)}")