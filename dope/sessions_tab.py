    ...
    try:
        ...
        if sessions:
            ...
            if session_summaries:
                ...
                st.info("💡 Select one or more rows to view combined metrics and graphs")
                ...
                if selected_rows.selection.rows:
                    ...
                    selected_sessions = [session_lookup[i] for i in selected_indices if i in session_lookup]
                    
                    if selected_sessions:
                        st.markdown("---")
                        st.markdown(f"### Combined Metrics ({len(selected_sessions)} sessions selected)")
                        ...
                        for session in selected_sessions:
                            ...
                            if measurements:
                                for measurement in measurements:
                                    ...
                                    measurement['session_name'] = session.get('session_name', 'N/A')
                                    measurement['bullet_type'] = session.get('bullet_type', 'N/A')
                                    measurement['bullet_grain'] = session.get('bullet_grain', 'N/A')
                                    measurement['created_at'] = session.get('created_at', 'N/A')
                                ...
                                session_summary = {
                                    ...
                                    'avg_velocity': sum(m['speed_fps'] for m in measurements if m.get('speed_fps')) / len(measurements) if measurements else 0,
                        ...
                        if all_measurements:
                            ...
                            with col4:
                                vel_spread = measurements_df['speed_fps'].max() - measurements_df['speed_fps'].min()
                            ...
                            tab1, tab2, tab3, tab4 = st.tabs(["Velocity Distribution", "Session Comparison", "Shot Sequence", "Statistics Summary"])
                            ...
                            with tab1:
                                ...
                                fig_hist = px.histogram(
                                    ...
                                    labels={'speed_fps': 'Velocity (fps)', 'count': 'Number of Shots'},
                                    nbins=20
                                )
                            ...
                            with tab2:
                                ...
                                fig_box = px.box(
                                    ...
                                    y='speed_fps',
                                    title='Velocity Distribution by Session',
                                    labels={'speed_fps': 'Velocity (fps)', 'session_name': 'Session'}
                                )
                            ...
                            with tab3:
                                ...
                                measurements_df['shot_sequence'] = measurements_df.groupby('session_name').cumcount() + 1
                                fig_line = px.line(
                                    measurements_df, 
                                    x='shot_sequence', 
                                    y='speed_fps',
                                    ...
                                    labels={'shot_sequence': 'Shot Number', 'speed_fps': 'Velocity (fps)'}
                                )
                            ...
                            with tab4:
                                ...
                                for session_summary in session_data:
                                    ...
                                    speeds = [m['speed_fps'] for m in measurements if m.get('speed_fps')]
                                    if speeds:
                                        stats_data.append({
                                            'Session': session_summary['session_name'],
                                            'Bullet Type': session_summary['bullet_type'],
                                            'Weight (gr)': session_summary['bullet_grain'],
