...
def get_unused_chrono_sessions(user_id: str, supabase):
    ...
    try:
        ...
        used_chrono_ids = {
            session.chrono_session_id for session in all_dope_sessions if session.chrono_session_id}

        # Filter out already used sessions
        unused_sessions = [
            session for session in all_chrono_sessions if session.id not in used_chrono_ids]
...
def get_rifles_for_user(_supabase, user_id: str):
    ...
    try:
        response = _supabase.table("rifles").select(
...
def get_unique_bullet_grains(cartridges):
    ...
    grains = set()
    for cartridge in cartridges:
        if cartridge.get("bullets") and cartridge["bullets"].get(
                "weight_grains"):
...
def filter_cartridges(
    ...
    if cartridge_type_filter and cartridge_type_filter != "All":
        filtered = [c for c in filtered if c.get(
            "cartridge_type") == cartridge_type_filter]

    # Filter by cartridge make
    if cartridge_make_filter and cartridge_make_filter != "All":
        filtered = [c for c in filtered if c.get(
            "make") == cartridge_make_filter]

    # Filter by bullet grain
    if bullet_grain_filter and bullet_grain_filter != "All":
        filtered = [c for c in filtered if c.get("bullets") and c["bullets"].get(
            "weight_grains") == bullet_grain_filter]
...
def render_step_1_chrono_selection(user_id: str, supabase):
    ...
    if not unused_sessions:
        ...
        st.write(
            "You need to upload chronograph data first before creating a DOPE session.")
        st.info(
            "💡 Go to the Chronograph page to upload your shooting data, then return here.")
        return None
    ...
    with st.expander("📊 Session Details", expanded=True):
        ...
        with col2:
            ...
            st.metric(
                "Avg Velocity",
                f"{selected_session.avg_speed_fps:.1f} fps")

        with col3:
            st.metric(
                "Std Deviation",
                f"{selected_session.std_dev_fps:.1f} fps")
            st.metric(
                "Date",
                selected_session.datetime_local.strftime('%Y-%m-%d'))
...
def render_step_2_rifle_selection(user_id: str, supabase):
    ...
    with st.expander("Rifle Details", expanded=True):
        ...
        with col1:
            ...
            st.write(
                f"**Cartridge Type:** {selected_rifle.get('cartridge_type', 'Not specified')}")

        with col2:
            st.write(
                f"**Barrel Length:** {selected_rifle.get('barrel_length', 'Not specified')}")
            st.write(
                f"**Twist Rate:** {selected_rifle.get('barrel_twist_ratio', 'Not specified')}")

    return selected_rifle


def render_step_3_cartridge_selection(
        user_id: str,
        supabase,
    ...
    st.write(
        f"Showing cartridges compatible with your rifle's cartridge type: **{rifle_cartridge_type}**")
    ...
    if rifle_cartridge_type:
        compatible_cartridges = [c for c in all_cartridges if c.get(
            "cartridge_type") == rifle_cartridge_type]
    else:
        compatible_cartridges = all_cartridges

    if not compatible_cartridges:
        st.warning(
            f"⚠️ No cartridges found that match your rifle's cartridge type: {rifle_cartridge_type}")
        st.write("You need to create a compatible cartridge first.")
        st.info(
            "💡 **How to create a compatible cartridge:**\n"
            "1. Go to the **Cartridges** page in the navigation menu\n"
            "2. Click on the **Create** tab\n"
    ...
    with col1:
        # Default to rifle's cartridge type, but allow "All" if user wants to
        # see everything
        default_index = 0
    ...
    bullet_grain_value = None if bullet_grain_filter == "All" else float(
        bullet_grain_filter.replace("gr", ""))
    ...
    with st.expander("Cartridge Details", expanded=True):
        ...
        with col1:
            st.write(
                f"**Cartridge Type:** {selected_cartridge.get('cartridge_type', 'Not specified')}")
            st.write(f"**Make:** {selected_cartridge.get('make', 'Unknown')}")
            st.write(
                f"**Model:** {selected_cartridge.get('model', 'Unknown')}")

        with col2:
            bullet = selected_cartridge.get("bullets", {})
            if bullet:
                st.write(
                    f"**Bullet:** {bullet.get('manufacturer', 'Unknown')} {bullet.get('model', 'Unknown')}")
                st.write(
                    f"**Weight:** {bullet.get('weight_grains', 'Unknown')} gr")
            else:
    ...
    if selected_range:
        with st.expander("Range Details", expanded=True):
            ...
            with col1:
                ...
                st.write(
                    f"**Distance:** {selected_range.get('distance_m', 'Unknown')} meters")
                st.write(
                    f"**Status:** {selected_range.get('status', 'Unknown')}")
            ...
            with col2:
                ...
                if selected_range.get('elevation_angle_deg'):
                    st.write(
                        f"**Elevation:** {selected_range['elevation_angle_deg']}°")

    return selected_range


def render_step_5_weather_selection(
        user_id: str,
        supabase,
    ...
    if not weather_sources:
        ...
        st.info(
            "You can create weather sources in the Weather page, or skip this step.")
    else:
...
def render_step_7_confirmation(
        chrono_session,
        rifle,
        cartridge,
        range_data,
    ...
    with st.expander("Session Summary", expanded=True):
        ...
        with col1:
            ...
            st.write(
                f"- Bullet: {chrono_session.bullet_type} ({chrono_session.bullet_grain}gr)")
            st.write(
                f"- Date: {chrono_session.datetime_local.strftime('%Y-%m-%d %H:%M')}")
            st.write(f"- Shots: {chrono_session.shot_count}")

            st.write("**Rifle:**")
            st.write(f"- Name: {rifle['name']}")
            st.write(
                f"- Cartridge: {rifle.get('cartridge_type', 'Not specified')}")

            st.write("**Range:**")
            if range_data:
                st.write(f"- Name: {range_data['range_name']}")
                st.write(
                    f"- Distance: {range_data.get('distance_m', 'Unknown')} meters")
            else:
        ...
        with col2:
            ...
            st.write(
                f"- Make/Model: {cartridge.get('make', 'Unknown')} {cartridge.get('model', 'Unknown')}")
            ...
            st.write("**Session:**")
            st.write(
                f"- Name: {session_details['session_name'] or 'Not specified'}")
            st.write(
                f"- Notes: {session_details['notes'][:50] + '...' if len(session_details.get('notes', '')) > 50 else session_details.get('notes', 'None')}")
...
def render_create_page(user, supabase):
    ...
    st.write(
        "Follow the steps below to create a new DOPE (Data On Previous Engagement) session.")
    ...
    for i, (col, step_name) in enumerate(
            zip(progress_cols, progress_steps), 1):
        with col:
    ...
    if current_step == 1:
        ...
        if result:
            ...
            try:
                ...
                time_window = weather_associator.get_chrono_session_time_window(
                    user["id"], result.id)
                dope_create_state["wizard_data"]["time_window"] = time_window
            except Exception as e:
                st.warning(
                    f"Could not determine time window for weather filtering: {str(e)}")
        ...
        result = render_step_3_cartridge_selection(
            user["id"],
            supabase,
            dope_create_state["wizard_data"]["rifle"].get("cartridge_type"))
        if result:
        ...
        result = render_step_5_weather_selection(
            user["id"], supabase, time_window)
        ...
        with col2:
            if st.button("Create DOPE Session", type="primary"):
                try:
                    ...
                    # Process weather association if weather source was
                    # selected
                    weather_association_results = None
                    if weather_data and time_window:
                        try:
                            weather_associator = WeatherSessionAssociator(
                                supabase)
                            weather_association_results = weather_associator.associate_weather_with_dope_session(
                                user["id"], new_session.id, weather_data.id, time_window[0], time_window[1])

                            if weather_association_results.get("error"):
                                st.warning(
                                    f"⚠️ Weather association warning: {weather_association_results['error']}")
                            else:
                                # Update DOPE session with median weather
                                # values
                                median_weather = weather_association_results.get(
                                    "median_weather", {})
                                if median_weather:
                                    weather_update_data = {}

                                    # Map median weather values to dope_session
                                    # fields
                                    field_mapping = {
                                    ...
                                    if weather_update_data:
                                        # Update the DOPE session with median
                                        # weather values
                                        supabase.table("dope_sessions").update(
                                            weather_update_data).eq("id", new_session.id).execute()

                                st.success(
                                    f"🌤️ Weather data processed: {weather_association_results['weather_measurement_count']} measurements")
                                st.success(
                                    f"🎯 Shot associations: {weather_association_results['associations_made']} of {weather_association_results['dope_measurement_count']} shots")

                        except Exception as weather_error:
                            st.warning(
                                f"⚠️ Weather association failed: {str(weather_error)}")
                            st.info(
                                "DOPE session created successfully, but weather data could not be processed.")
        ...
        if created_session:
            ...
            if weather_results and not weather_results.get("error"):
                st.success(
                    f"🌤️ Weather data processed: {weather_results['weather_measurement_count']} measurements")
                st.success(
                    f"🎯 Shot associations: {weather_results['associations_made']} of {weather_results['dope_measurement_count']} shots")
                st.info("Weather measurements have been associated with your shots.")
            elif weather_results and weather_results.get("error"):
                st.warning(
                    f"⚠️ Weather association warning: {weather_results['error']}")
                st.info(
                    "DOPE session created successfully, but weather data could not be processed.")
