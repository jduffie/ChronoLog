...
class DopeCreateBusiness:
    ...
    def filter_cartridges_by_rifle_type(
        self, cartridges, rifle_cartridge_type: str
    ) -> List[CartridgeModel]:
    ...
    def filter_cartridges(
        ...
        if cartridge_type_filter and cartridge_type_filter != "All":
            filtered = [
                c for c in filtered if c.cartridge_type == cartridge_type_filter
            ]
        ...
        if bullet_grain_filter and bullet_grain_filter != "All":
            filtered = [
                c for c in filtered if c.bullet_weight_grains == bullet_grain_filter
            ]
    ...
    def update_session_with_weather_data(
        self, session_id: str, weather_association_results: dict
    ):
        ...
        try:
            ...
            if weather_update_data:
                ...
                self.supabase.table("dope_sessions").update(weather_update_data).eq(
                    "id", session_id
                ).execute()
    ...
    def prepare_session_data(
        ...
        bullet_id = (
            cartridge.bullet_id
            if hasattr(cartridge, "bullet_id")
            else cartridge.get("bullet_id")
        )
        if not cartridge_id:
        ...
        if not time_window or len(time_window) != 2:
            raise ValueError(
                "Session time window (start_time and end_time) is required"
            )
        ...
        return {
            ...
            "start_altitude": (
                range_data.get("start_altitude_m") if range_data else None
            ),
            "azimuth_deg": range_data.get("azimuth_deg") if range_data else None,
            "elevation_angle_deg": (
                range_data.get("elevation_angle_deg") if range_data else None
            ),
            "location_hyperlink": (
                f"https://maps.google.com/maps?q={range_data.get('start_lat')},{range_data.get('start_lon')}"
                if range_data
                and range_data.get("start_lat")
                and range_data.get("start_lon")
                else None
            ),
