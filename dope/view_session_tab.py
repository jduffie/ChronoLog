...
def render_view_session_tab(user, supabase):
    ...
    try:
        ...
        sessions_response = supabase.table("dope_sessions").select("""
            *, 
            ...
            rifles(name),
            ammo(make, model)
        """).eq("user_email", user["email"]).order("created_at", desc=True).execute()
        ...
        if sessions:
            ...
            for session in sessions:
                ...
                if ammo_data:
                    ammo_name = f"{ammo_data.get('make', '')} {ammo_data.get('model', '')}".strip() or 'N/A'
            ...
            if session_summaries:
                ...
                with st.expander("🔍 Search & Filter", expanded=False):
                    ...
                    with col1:
                        ...
                        unique_ranges = sorted([r for r in sessions_df['Range'].unique() if r != 'N/A'])
                    ...
                    with col2:
                        ...
                        unique_rifles = sorted([r for r in sessions_df['Rifle'].unique() if r != 'N/A'])
                    ...
                    with col3:
                        ...
                        unique_ammo = sorted([a for a in sessions_df['Ammo'].unique() if a != 'N/A'])
                ...
                for new_index, (original_index, row) in enumerate(filtered_df.iterrows()):
                    if original_index in session_lookup:
                        filtered_session_lookup[new_index] = session_lookup[original_index]
                
                # Show filter results info
                if len(filtered_df) != len(sessions_df):
                    st.info(f"🔍 Showing {len(filtered_df)} of {len(sessions_df)} sessions")
                ...
                if selected_rows.selection.rows:
                    ...
                    if selected_row_index in filtered_session_lookup:
                        ...
                        chrono_session_id = selected_session.get('chrono_session_id', 'N/A')
                        chrono_session = selected_session.get('chrono_sessions')
                        chrono_datetime = chrono_session.get('datetime_local', 'N/A') if chrono_session else 'N/A'
                        ...
                        if not view_model.is_tab_created(tab_name):
                            load_session_into_cache(view_model, selected_session, tab_name)
                        
                        # Get cached session data (includes any pending edits)
                        cached_session_details = view_model.get_tab_session_details(tab_name)
                        ...
                        with st.expander("✏️ Edit Optional Sources", expanded=False):
                            edit_optional_sources_cached(user, supabase, view_model, tab_name, cached_session_details)
                        
                        # Session info (use cached data which includes any pending edits)
                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            st.metric("Session Name", cached_session_details.get('session_name', 'N/A'))
                        with col2:
                            st.metric("Bullet Type", cached_session_details.get('bullet_type', 'N/A'))
                        with col3:
                            st.metric("Bullet Weight", f"{cached_session_details.get('bullet_grain', 'N/A')} gr")
                        with col4:
                            # Use datetime_local from chrono_sessions for the session date/time
                            chrono_session = selected_session.get('chrono_sessions')  # This metadata doesn't change
                            if chrono_session and chrono_session.get('datetime_local'):
                                session_dt = pd.to_datetime(chrono_session['datetime_local'])
                                st.metric("Session Date", session_dt.strftime('%Y-%m-%d %H:%M'))
                            elif cached_session_details.get('created_at'):
                                # Fallback to created_at
                                created_dt = pd.to_datetime(cached_session_details['created_at'])
                        ...
                        if ammo_data:
                            ...
                            if selected_session.get('ammo_id'):
                                ...
                                if ammo_response.data:
                                    ...
                                    col1, col2, col3, col4 = st.columns(4)
                                    with col1:
                                        st.markdown(f"<medium><strong>Make:</strong> {full_ammo_data.get('make', 'N/A')}</medium>", unsafe_allow_html=True)
                                    with col2:
                                        st.markdown(f"<medium><strong>Model:</strong> {full_ammo_data.get('model', 'N/A')}</medium>", unsafe_allow_html=True)
                                    with col3:
                                        st.markdown(f"<medium><strong>Caliber:</strong> {full_ammo_data.get('caliber', 'N/A')}</medium>", unsafe_allow_html=True)
                                    with col4:
                                        st.markdown(f"<medium><strong>Weight:</strong> {full_ammo_data.get('weight', 'N/A')}</medium>", unsafe_allow_html=True)
                                else:
                        ...
                        if rifle_data and selected_session.get('rifle_id'):
                            ...
                            if rifle_response.data:
                                ...
                                col1, col2, col3, col4 = st.columns(4)
                                with col1:
                                    st.markdown(f"<medium><strong>Name:</strong> {full_rifle_data.get('name', 'N/A')}</medium>", unsafe_allow_html=True)
                                with col2:
                                    st.markdown(f"<medium><strong>Barrel Length:</strong> {full_rifle_data.get('barrel_length', 'N/A')}</medium>", unsafe_allow_html=True)
                                with col3:
                                    st.markdown(f"<medium><strong>Twist Ratio:</strong> {full_rifle_data.get('barrel_twist_ratio', 'N/A')}</medium>", unsafe_allow_html=True)
                                with col4:
                                    st.markdown(f"<medium><strong>Scope:</strong> {full_rifle_data.get('scope', 'N/A')}</medium>", unsafe_allow_html=True)
                            else:
                        ...
                        if cached_session_details.get('range_submission_id'):
                            range_response = supabase.table("ranges_submissions").select("*").eq("id", cached_session_details['range_submission_id']).execute()
                            if range_response.data:
                                full_range_data = range_response.data[0]
                                col1, col2, col3 = st.columns(3)
                                with col1:
                                    lat_text = f"{full_range_data.get('start_lat', 'N/A'):.6f}" if full_range_data.get('start_lat') else "N/A"
                                    st.markdown(f"<medium><strong>Latitude:</strong> {lat_text}</medium>", unsafe_allow_html=True)
                                with col2:
                                    lon_text = f"{full_range_data.get('start_lon', 'N/A'):.6f}" if full_range_data.get('start_lon') else "N/A"
                                    st.markdown(f"<medium><strong>Longitude:</strong> {lon_text}</medium>", unsafe_allow_html=True)
                                with col3:
                                    alt_text = f"{full_range_data.get('start_altitude_m', 'N/A'):.1f}" if full_range_data.get('start_altitude_m') else "N/A"
...
def edit_optional_sources(user, supabase, selected_session):
    ...
    try:
        ...
        with col1:
            ...
            if weather_sources.data:
                for i, weather_source in enumerate(weather_sources.data):
                    ...
                    if current_weather_id and weather_source['id'] == current_weather_id:
        ...
        changes_made = (
            new_range_id != current_range_id or
            new_weather_id != current_weather_id or
            new_rifle_id != current_rifle_id or
            new_ammo_id != current_ammo_id
        )
        ...
        # Update button
        if st.button("🔄 Update Session", type="primary", key=f"update_session_{selected_session['id']}"):
            try:
                ...
                response = supabase.table("dope_sessions").update(update_data).eq("id", selected_session['id']).execute()
...
def edit_optional_sources_cached(user, supabase, view_model, tab_name, cached_session_details):
    ...
    try:
        ...
        if changes_made:
            ...
            st.info("💡 Changes are visible immediately. Click 'Save to Database' to persist changes.")
        
        # Check if there are any unsaved changes in cache vs database
        session_response = supabase.table("dope_sessions").select("range_submission_id, weather_source_id, rifle_id, ammo_id").eq("id", cached_session_details['session_id']).execute()
        if session_response.data:
            db_data = session_response.data[0]
            unsaved_changes = (
                cached_session_details.get('range_submission_id') != db_data.get('range_submission_id') or
                cached_session_details.get('weather_source_id') != db_data.get('weather_source_id') or
                cached_session_details.get('rifle_id') != db_data.get('rifle_id') or
                cached_session_details.get('ammo_id') != db_data.get('ammo_id')
            )
        ...
        if st.button("💾 Save to Database", type="primary", key=f"save_cached_session_{cached_session_details['session_id']}"):
            try:
                # Update the DOPE session with cached values
                update_data = {
                    "range_submission_id": cached_session_details.get('range_submission_id'),
                    "weather_source_id": cached_session_details.get('weather_source_id'),
                    "rifle_id": cached_session_details.get('rifle_id'),
                    "ammo_id": cached_session_details.get('ammo_id'),
                    "updated_at": datetime.now().isoformat()
                }
                
                response = supabase.table("dope_sessions").update(update_data).eq("id", cached_session_details['session_id']).execute()
