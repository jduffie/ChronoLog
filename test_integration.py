        ...
        if not self.mock_mode:
            ...
            for session_id in self.test_session_ids:
                try:
                    ...
                    self.supabase.table('measurements').delete().eq('session_id', session_id).execute()
                except Exception:
    ...
    def test_complete_upload_workflow(self):
        ...
        try:
            ...
            if self.mock_mode:
                ...
                self.supabase.table.return_value.insert.return_value.execute.return_value = mock_session_response
                
                mock_measurements_response = Mock() 
                mock_measurements_response.data = [{'id': f'measurement-{i}'} for i in range(5)]
                self.supabase.table.return_value.insert.return_value.execute.return_value = mock_measurements_response
            
            # Read and process the Excel file
            df = pd.read_excel(excel_file_path, sheet_name='Sheet1', skiprows=2)
            bullet_info = pd.read_excel(excel_file_path, sheet_name='Sheet1', nrows=1).iloc[0, 0]
...
class TestCrossModuleIntegration(BaseIntegrationTest):
    ...
    def test_chronograph_data_consistency(self):
        ...
        if self.mock_mode:
            ...
            self.supabase.table.return_value.insert.return_value.execute.return_value = mock_response
...
class TestAuthenticationIntegration(BaseIntegrationTest):
    ...
    def test_auth_to_data_access_flow(self, mock_session_state):
        ...
        if self.mock_mode:
            ...
            self.supabase.table.return_value.select.return_value.eq.return_value.execute.return_value = mock_response
        
        # Verify that data access is user-scoped
        # In real implementation, this would query actual database
        user_sessions = []  # chronograph_service.get_sessions_for_user(self.test_user_email)
