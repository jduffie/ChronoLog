...
from mapping.test_mapping import TestPublicRangesController, TestMappingPageStructure, TestMappingModels
    ...
    def test_all_pages_exist(self):
        ...
        for page_file in expected_pages:
            with self.subTest(page=page_file):
                self.assertTrue(os.path.exists(page_file), f"Page file {page_file} should exist")
            ...
            with self.subTest(page=page_file):
                if os.path.exists(page_file):
                    ...
                    for required_import in required_imports:
                        self.assertIn(required_import, content,
        ...
        for page_file in page_files:
            with self.subTest(page=page_file):
                if os.path.exists(page_file):
                    ...
                    self.assertTrue(has_main or has_run, 
...
class TestMainAppStructure(unittest.TestCase):
    ...
    def test_chronolog_imports_home_page(self):
        ...
        if os.path.exists("ChronoLog.py"):
            ...
            self.assertIn("pages/1_🏠_Home.py", content,
                        "ChronoLog.py should import the home page")
            self.assertIn("importlib.util", content,
...
class TestPageTabStructure(unittest.TestCase):
    ...
    def test_weather_has_correct_tabs(self):
        ...
        if os.path.exists("pages/4_🌤️_Weather.py"):
            ...
            for tab in expected_tabs:
                self.assertIn(f'"{tab}"', content,
                            f"Weather page should have {tab} tab")
    ...
    def test_dope_has_correct_tabs(self):
        ...
        if os.path.exists("pages/2_📊_DOPE.py"):
            ...
            for tab in expected_tabs:
                self.assertIn(f'"{tab}"', content,
                            f"DOPE page should have {tab} tab")
        ...
        if os.path.exists("pages/5_🌍_Ranges.py"):
            ...
            for pattern in tab_patterns:
                self.assertIn(pattern, content,
                            f"Ranges page should reference {pattern}")
        ...
        for page_file in pages_with_db:
            with self.subTest(page=page_file):
                if os.path.exists(page_file):
                    ...
                    self.assertIn('st.secrets["supabase"]', content,
