    ...
    try:
        ...
        with col4:
            ...
            if "country" in df.columns and not df["country"].empty:
                ...
                country_count = (
                    (df["country"] == most_common_country).sum()
                    if most_common_country != "N/A"
                    else 0
                )
        ...
        with col2:
            unit_systems = (
                ["All"] + sorted(df["unit_system"].dropna().unique().tolist())
                if "unit_system" in df.columns
                else ["All"]
            )
        ...
        for col in date_columns:
            if col in display_df.columns:
                display_df[f"{col}_display"] = pd.to_datetime(
                    display_df[col]
        ...
        table_df = display_df[available_columns].rename(
            columns={
                "email": "Email",
                "name": "Name",
                "username": "Username",
...
def render_user_edit_form(user_data, supabase):
    ...
    st.markdown(
        f"**Editing user:** {user_data.get('name', 'Unknown')} ({user_data.get('email', 'Unknown')})"
    )
    ...
    with st.form(key=f"edit_user_{user_data.get('id')}"):
        ...
        with col1:
            ...
            new_username = st.text_input(
                "Username:", value=user_data.get("username", "")
            )
        ...
        with col2:
            ...
            new_unit_system = st.selectbox(
                ...
                index=(
                    unit_system_options.index(current_unit_system)
                    if current_unit_system in unit_system_options
                    else 0
                ),
            )
...
def render_user_delete_form(user_data, supabase):
    ...
    st.markdown(
        f"**Delete user:** {user_data.get('name', 'Unknown')} ({user_data.get('email', 'Unknown')})"
    )
    ...
    if delete_button and email_matches:
        try:
            ...
            if delete_response.data:
                st.success(
                    f"✅ User '{user_data.get('name', 'Unknown')}' has been permanently deleted."
                )
            ...
            if (
                ...
                or "violates foreign key" in str(e).lower()
            ):
                st.error(
                    "🔗 **Cannot delete user:** This user has associated data in other tables."
                )
                st.info(
                    "💡 **Suggestion:** You may need to delete or transfer the user's data first before deleting the user account."
                )
